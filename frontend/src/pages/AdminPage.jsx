...
function ChuyenLopTab({ toast }) {
  ...
  const load = useCallback(async () => {
    setLoading(true);
    try {
      const data = await transferAPI.getPending();
      console.log("Dữ liệu chuyển lớp:", data); // 👈 xem cấu trúc thật
      setRows(toList(data));
    } catch (e) { toast(e.message, "error"); }
    finally { setLoading(false); }
  }, [toast]);
  ...
  const approve = async (id) => {
    try {
      ...
      toast("Đã chấp nhận chuyển lớp thành công!", "info");
      load();
    } catch (e) { toast(e.message, "error"); }
  };

  // Helper lấy tên sinh viên (giống fullName trong SinhVienTab)
  const getStudentName = (sv) => {
    if (!sv) return "—";
    const ho = sv.ho || "";
    const ten = sv.ten || "";
    return `${ho} ${ten}`.trim() || (sv.mssv || "—");
  };

  // Helper lấy mã lớp hiện tại (giống getStudentMalop)
  const getCurrentClass = (sv) => {
    if (!sv) return "—";
    if (sv.lop && typeof sv.lop === "object") return sv.lop.malop || "—";
    if (sv.malop) return sv.malop;
    return "—";
  };
      ...
      <div className="admin-topbar">
        <div>
          <h2 className="page-title">
            <span className="dot" style={{ color: "var(--accent)" }}>●</span>
            Yêu cầu chuyển lớp
          </h2>
          <p className="page-subtitle">{rows.length} yêu cầu đang chờ duyệt</p>
        </div>
      </div>
      ...
      <div className="table-card">
        {loading ? (
          ...
          <div className="empty-state">
            ...
            <p>Không có yêu cầu chuyển lớp nào</p>
          </div>
        ) : (
          <table className="data-table">
            <thead>
            ...
            <tbody>
              {rows.map((r, i) => {
                // Lấy object sinh viên từ các trường có thể có
                const sv = r.sinhVienT || r.sinhVien || r.student || null;

                // MSSV: ưu tiên từ object sinh viên, nếu không thì từ trực tiếp
                const mssv = sv?.mssv || r.mssv || "—";

                // Tên sinh viên
                const hoTen = sv ? getStudentName(sv) : (r.hoTen || r.tensv || mssv);

                // Lớp hiện tại: ưu tiên từ object sinh viên, nếu không thì từ các trường khác
                const lopHienTai = sv
                  ? getCurrentClass(sv)
                  : (r.malopcu || r.malopHienTai || r.lopCu || r.lopHienTai || r.currentClass || "—");

                // Lớp muốn chuyển: thử nhiều tên trường
                const lopMoi = r.malopmoi || r.malopMoi || r.lopMoi || r.targetClass || r.requestedClass || "—";

                // Lý do
                const lyDo = r.reason || r.lydo || "—";
                ...
                return (
                  <tr key={r.id || i}>
                    ...
                    <td>
                      <strong>{hoTen}</strong>
                      <div style={{ fontSize: 11, color: "var(--dim)" }}>{mssv}</div>
                    </td>
                    <td><span className="badge badge-normal">{lopHienTai}</span></td>
                    <td><span className="badge badge-green">{lopMoi}</span></td>
                    <td style={{ color: "var(--muted)", maxWidth: 200 }}>{lyDo}</td>
                    <td>
...
const TABS = [
  ...
  { id: "chuyenlop", label: "Chuyển Lớp", icon: "" },
];
  ...
  return (
    <div className="admin-layout">
      ...
      <main className="admin-main">
        ...
        {tab === "chuyenlop" && <ChuyenLopTab toast={toast} />}
      </main>
    </div>
  );
}